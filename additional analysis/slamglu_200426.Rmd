...
main_all2 <- main_all2 %>% 
  ...
  mutate(per_age_wk = 
         ...
         #individual 
         per_age_wk2 = (age_wk / as.numeric(le_wk))*100
  ) %>% 
  filter(per_age_wk2 <= 100) %>% 
  select(-fu_age_wk, -le_wk, -cod, -dead_censor)
...
census_c16_death <- census_c16_death %>% 
  mutate(per_fu_wk = 
         ...
         #individual 
         per_fu_wk2 = (fu_age_wk / as.numeric(le_wk))*100, 
         per_le_wk2 = (le_wk / as.numeric(le_wk))*100
  ) 
...
## by percent age 
allgluc_perage_range <- format(round(as.data.frame(psych::describe(main_all2_desc_gluc$per_age_wk2)), 2), nsmall=2) %>% 
  mutate(group1 = "all") %>% 
  select(group1, le_min = min, le_max = max)
allbw_perage_range <- format(round(as.data.frame(psych::describe(main_all2_desc_bw$per_age_wk2)), 2), nsmall=2) %>% 
  mutate(group1 = "all") %>% 
  select(group1, le_min = min, le_max = max)
allfat_perage_range <- format(round(as.data.frame(psych::describe(main_all2_desc_fat$per_age_wk2)), 2), nsmall=2) %>% 
  mutate(group1 = "all") %>% 
  select(group1, le_min = min, le_max = max)
#by sex
gluc_perage_range <- as.data.frame(describeBy(main_all2_desc_gluc$per_age_wk2, main_all2_desc_gluc$sex, mat=TRUE, digits = 2)) %>% 
  select(group1, le_min = min, le_max = max) 
bw_perage_range <- as.data.frame(describeBy(main_all2_desc_bw$per_age_wk2, main_all2_desc_bw$sex, mat=TRUE, digits = 2)) %>% 
  select(group1, le_min = min, le_max = max) 
fat_perage_range <- as.data.frame(describeBy(main_all2_desc_fat$per_age_wk2, main_all2_desc_fat$sex, mat=TRUE, digits = 2)) %>% 
  select(group1, le_min = min, le_max = max) 
...
gam_fit <- mgcv::gam(eval(parse(text = metric)) ~ s(per_age_wk2, bs = "cs"), method = "REML", data = peaks_df) 
...
fit2 <- peaks_df %>% 
  bind_cols(predict1) %>% 
  select(per_age_wk2, fit, se.fit) %>% 
  filter(per_age_wk2 >= low_bound, per_age_wk2 <= high_bound) %>%
  group_by(per_age_wk2) %>% 
  mutate( 
    n = n(), 
    sd = se.fit*sqrt(n) 
  ) %>% 
...
low_bound_peak <- fit2 %>% 
  filter(per_age_wk2 == min(per_age_wk2)) %>% 
  select(per_age_wk2, fit, se.fit, n, sd) %>% 
  distinct()

#create high bound peak object
high_bound_peak <- fit2 %>% 
  filter(per_age_wk2 == max(per_age_wk2)) %>% 
  select(per_age_wk2, fit, se.fit, n, sd) %>% 
  distinct()
...
peak <- fit2 %>% 
  ...
  group_by(per_age_wk2, n, sd, se.fit) %>%
  summarise(
    fit = max(fit)
  )  %>% 
  ungroup() %>% 
...
fit2 %>% 
  filter(per_age_wk2 < peak$per_age_wk2) %>% 
  mutate(fit_diff = fit - peak$fit) %>% 
  mutate(age_diff = peak$per_age_wk2 - per_age_wk2) %>% 
...
fit2 %>% 
  filter(per_age_wk2 > peak$per_age_wk2) %>% 
  mutate(fit_diff = fit - peak$fit) %>% 
  mutate(age_diff = per_age_wk2 - peak$per_age_wk2) %>% 
  mutate(pval = tsum.test(fit_diff, sd, n, mu = 0, conf.level = 0.95)$p.value) %>% 
  filter(pval <= 0.05) %>% 
  filter( 
    age_diff == min(age_diff) 
...
#age adjustment 
main_cat2 <- carry_forward(quote(per_age_wk2), quote(25))
main_cat2 <- carry_forward(quote(per_age_wk2), quote(50))
main_cat2 <- carry_forward(quote(per_age_wk2), quote(75))
main_cat2 <- carry_forward(quote(per_age_wk2), quote(100))
main_cat2 <- carry_forward(quote(per_age_wk2), quote(125))
...
fcn_plotdist_smooth <- function(metric, x, y, by, xlabel, ylabel, title, xlow, xhigh, peak1, peak2) { 
  ...
  main_all3 <- main_all3 %>% 
    filter(per_age_wk2 < as.numeric(all_asmt$le_max[1]))
  ...
  ggplot(subset(df, eval(parse(text = sub))), aes_string(x = x, y = y, colour = by)) + 
    ...
    #peaks
    geom_point(aes(x = peak1$all$per_age_wk2, y = peak1$all$fit), size=2, color="black", fill = "white", alpha=0.7, shape=23, stroke=2) +
    geom_point(aes(x = peak2$all$per_age_wk2, y = peak2$all$fit), size=2, color="black", fill = "white", alpha=0.7, shape=23, stroke=2) +
...
#sex
slam_fig_i1 <- fcn_plotdist_smooth(metric = "gluc", x = "per_age_wk2", xlabel = "Percent Life Expectancy", xlow = 0, xhigh = 100, 
...
slam_fig_i2 <- fcn_plotdist_smooth(metric = "bw", x = "per_age_wk2", xlabel = "Percent Life Expectancy", xlow = 0, xhigh = 100, 
                        y = "bw", ylabel = "Body Weight (g)", by = "sex", peak1 = bw_peak_male, peak2 = bw_peak_female) + 
  scale_color_manual(labels = c("Female", "Male"), name = "Sex", values = cbPalette_dot)
slam_fig_i3 <- fcn_plotdist_smooth(metric = "fat", x = "per_age_wk2", xlabel = "Percent Life Expectancy", xlow = 0, xhigh = 100, 
                        y = "fat", ylabel = "Body Fat (g)", by = "sex", peak1 = fat_peak_male, peak2 = fat_peak_female) + 
  scale_color_manual(labels = c("Female", "Male"), name = "Sex", values = cbPalette_dot)

#strain
slam_fig_j1 <- fcn_plotdist_smooth(metric = "gluc", x = "per_age_wk2", xlabel = "Percent Life Expectancy", xlow = 0, xhigh = 100, 
...
slam_fig_j2 <- fcn_plotdist_smooth(metric = "bw", x = "per_age_wk2", xlabel = "Percent Life Expectancy", xlow = 0, xhigh = 100, 
                        y = "bw", ylabel = "Body Weight (g)", by = "strain", peak1 = bw_peak_HET3, peak2 = bw_peak_B6) + 
  scale_color_manual(labels = c("B6", "HET3"), name = "Strain", values = cbPalette_dot)
slam_fig_j3 <- fcn_plotdist_smooth(metric = "fat", x = "per_age_wk2", xlabel = "Percent Life Expectancy", xlow = 0, xhigh = 100, 
                        y = "fat", ylabel = "Body Fat (g)", by = "strain", peak1 = fat_peak_HET3, peak2 = fat_peak_B6) + 
  scale_color_manual(labels = c("B6", "HET3"), name = "Strain", values = cbPalette_dot)

#death
slam_fig_d1 <- fcn_plotdist_lm(metric = "gluc", x = "per_age_wk2", xlabel = "Percent Life Expectancy", xlow = 0, xhigh = 100, 
                        y = "gluc", ylabel = "Glucose (mg/dL)", by = "whendead")
slam_fig_d2 <- fcn_plotdist_lm(metric = "bw", x = "per_age_wk2", xlabel = "Percent Life Expectancy", xlow = 0, xhigh = 100, 
                        y = "bw", ylabel = "Body Weight (g)", by = "whendead") 
slam_fig_d3 <- fcn_plotdist_lm(metric = "fat", x = "per_age_wk2", xlabel = "Percent Life Expectancy", xlow = 0, xhigh = 100, 
...
slam_km_gluc_50 <- slam_surv_gluc_50_all[[1]]$plot + ylim(-0.05, 1) + 
  annotate("text", x = -Inf, y = -0.05, hjust = -.09, label = deparse(bquote(HR[adj]== .(slam_surv_gluc_50_all[[2]]))), size = 4, parse =TRUE, family = nytimes)
slam_km_gluc_75 <- slam_surv_gluc_75_all[[1]]$plot + ylim(-0.05, 1) + 
  annotate("text", x = -Inf, y = -0.05, hjust = -.09, label = deparse(bquote(HR[adj]== .(slam_surv_gluc_75_all[[2]]))), size = 4, parse =TRUE, family = nytimes)
slam_km_gluc_100 <- slam_surv_gluc_100_all[[1]]$plot + ylim(-0.05, 1) + 
  annotate("text", x = -Inf, y = -0.05, hjust = -.09, label = deparse(bquote(HR[adj]== .(slam_surv_gluc_100_all[[2]]))), size = 4, parse =TRUE, family = nytimes)

slam_km_bw_50 <- slam_surv_bw_50_all[[1]]$plot + ylim(-0.05, 1) + 
  annotate("text", x = -Inf, y = -0.05, hjust = -.09, label = deparse(bquote(HR[adj]== .(slam_surv_bw_50_all[[2]]))), size = 4, parse =TRUE, family = nytimes)
slam_km_bw_75 <- slam_surv_bw_75_all[[1]]$plot + ylim(-0.05, 1) + 
  annotate("text", x = -Inf, y = -0.05, hjust = -.09, label = deparse(bquote(HR[adj]== .(slam_surv_bw_75_all[[2]]))), size = 4, parse =TRUE, family = nytimes)
slam_km_bw_100 <- slam_surv_bw_100_all[[1]]$plot + ylim(-0.05, 1) + 
  annotate("text", x = -Inf, y = -0.05, hjust = -.09, label = deparse(bquote(HR[adj]== .(slam_surv_bw_100_all[[2]]))), size = 4, parse =TRUE, family = nytimes)

slam_km_fat_50 <- slam_surv_fat_50_all[[1]]$plot + ylim(-0.05, 1) + 
  annotate("text", x = -Inf, y = -0.05, hjust = -.09, label = deparse(bquote(HR[adj]== .(slam_surv_fat_50_all[[2]]))), size = 4, parse =TRUE, family = nytimes)
slam_km_fat_75 <- slam_surv_fat_75_all[[1]]$plot + ylim(-0.05, 1) + 
  annotate("text", x = -Inf, y = -0.05, hjust = -.09, label = deparse(bquote(HR[adj]== .(slam_surv_fat_75_all[[2]]))), size = 4, parse =TRUE, family = nytimes)
slam_km_fat_100 <- slam_surv_fat_100_all[[1]]$plot + ylim(-0.05, 1) + 
  annotate("text", x = -Inf, y = -0.05, hjust = -.09, label = deparse(bquote(HR[adj]== .(slam_surv_fat_100_all[[2]]))), size = 4, parse =TRUE, family = nytimes)
