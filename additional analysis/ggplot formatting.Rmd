...
bodywt <- bodywt %>% 
  ...
  bodywt$date <- as.Date(as.character(bodywt$date), "%m/%d/%Y")
...
bodycomp <- read.csv("data/slam/NMR.csv") %>% 
...
annotate_figure(slam_fig_j13, 
                bottom = text_grob("Note: \nHorizontal lines indicate percent of life expectancy: green (25%), orange (50%), red (75%), purple (100%), and black (125%) \nDiamonds represent age at peak index value", size = 9, hjust = 0, x = unit(0, "lines")), 
                top = text_grob("Figure 1. Trends in Metabolic indices over the Lifespan by Strain \n", size = 12, hjust = 0, x = unit(0, "lines"))
)
