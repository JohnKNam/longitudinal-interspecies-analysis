...
```{r isg_km_hr, warning=FALSE, message=FALSE, echo=FALSE, eval=TRUE, include=TRUE}

#original method

# slam_hr_toc_by_3a$species <- "Mice (SLAM)"
# nhp_hr_toc_by_3a$species <- "Non-Human Primates"
# blsa_hr_toc_by_3a$species <- "Human (BLSA)"
# 
# slam_hr_toc_by_3b$species <- "Mice (SLAM)"
# nhp_hr_toc_by_3b$species <- "Non-Human Primates"
# blsa_hr_toc_by_3b$species <- "Human (BLSA)"

# isg_hr_glucadj <- bind_rows(slam_hr_toc_by_3a, nhp_hr_toc_by_3a, blsa_hr_toc_by_3a) %>% 
#   select(species, everything())
# 
# isg_hr_bw <- bind_rows(slam_hr_toc_by_3b, nhp_hr_toc_by_3b, blsa_hr_toc_by_3b) %>% 
#   filter(indices == "Body Weight") %>% 
#   select(species, everything())
# 
# isg_hr_fat <- bind_rows(slam_hr_toc_by_3b, nhp_hr_toc_by_3b, blsa_hr_toc_by_3b) %>% 
#   filter(indices == "Body Composition") %>% 
#   select(species, everything())
# 
# isg_hr_fat_bw <- bind_rows(isg_hr_bw, isg_hr_fat) %>% 
#   select(indices, species, everything())
# 
# isg_hr_fat_bw2 <- isg_hr_fat_bw %>% 
